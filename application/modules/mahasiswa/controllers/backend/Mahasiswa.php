...
/*| youtube :  */
/*| --------------------------------------------------------------------------*/
/*| Generate By M-CRUD Generator 12/12/2023 10:00*/
    ...
    foreach ($list as $row) {
        ...
        $rows[] = '
                  <div class="btn-group" role="group" aria-label="Basic example">
                      <a href="'.url("mahasiswa/detail/".enc_url($row->id)).'" id="detail" class="btn btn-primary" title="'.cclang("detail").'">
                        <i class="mdi mdi-file"></i>
                      </a>
                      <a href="'.url("mahasiswa/update/".enc_url($row->id)).'" id="update" class="btn btn-warning" title="'.cclang("update").'">
                        <i class="ti-pencil"></i>
                      </a>
                      <a href="'.url("mahasiswa/delete/".enc_url($row->id)).'" id="delete" class="btn btn-danger" title="'.cclang("delete").'">
                        <i class="ti-trash"></i>
                      </a>
                    </div>
                 ';
...
function add()
{
  ...
  $data = array('action' => url("mahasiswa/add_action"),
                  'nama' => set_value("nama"),
...
function update($id)
{
  ...
  if ($row = $this->model->find(dec_url($id))) {
    ...
    $data = array('action' => url("mahasiswa/update_action/$id"),
